...
public class ViewTVShowActivity extends AppCompatActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (!getIntent().hasExtra("tv_show_id") || !getIntent().hasExtra("tv_show_name")) finish();
		...
		StringRequest stringRequest = new StringRequest(Request.Method.POST, Constants.API_URL + "/shows/get/" + tvShowId, response1 -> {
			try {
				JSONObject jsonObject1 = new JSONObject(response1);
				if (!jsonObject1.getBoolean("error")) {
					
					JSONObject dataJO = jsonObject1.getJSONObject("data");
					imageLoader.displayImage(dataJO.getString("backdrop"), backdrop, new DisplayImageOptions.Builder().showImageOnLoading(R.drawable.no_backdrop_16_9).showImageOnFail(R.drawable.no_backdrop_16_9).displayer(new FadeInBitmapDisplayer(500)).cacheInMemory(false).cacheOnDisk(true).build());
					...
					for (int i = 0; i < seasons.length(); i++) {
						...
						checkBox.setOnCheckedChangeListener((compoundButton, checked) -> {
							if (checked) {
								...
								StringRequest stringRequest2 = new StringRequest(Request.Method.POST, Constants.API_URL + "/mark_season_as_watched", response2 -> {
									try {
								...
								StringRequest stringRequest3 = new StringRequest(Request.Method.POST, Constants.API_URL + "/remove_watched_season", response3 -> {
									try {
					...
					if (jsonObject1.getString("error_id").equals("movie_not_found")) finish();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
			...
			case R.id.add_to_favourites:
				StringRequest stringRequest2 = new StringRequest(Request.Method.POST, Constants.API_URL + "/shows/favourites/add", response -> {
