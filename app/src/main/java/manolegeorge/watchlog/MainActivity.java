...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		addFriendDialog.setOnShowListener(dialog -> {
			...
			positive.setOnClickListener(v -> {
				final String username1 = addFriendUsername.getText().toString().trim();
				if (username1.length() > 0) {
					
					StringRequest stringRequest = new StringRequest(Request.Method.POST, Constants.API_URL + "/friends/add", response -> {
